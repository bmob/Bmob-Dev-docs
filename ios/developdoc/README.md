...
+ 在 `[gameScore saveInBackgroundWithResultBlock:^(BOOL isSuccessful, NSError *error)`中，成功创建后，error返回的是nil，可以通过[[error userInfo] objectForKey:@"error"]查看返回的错误信息，之后的类似于 `xxInBackground` 中的error也是一样的结构。
...
![](image/relationship_1.jpg)

Post表

![](image/relationship_2.jpg)
...
```
    ...
    //设置帖子关联的作者记录
    BmobUser *author = [BmobUser objectWithoutDatatWithClassName:@"_User" objectId:@"2bc3d712fb"];
...
```
    ...
    [bquery getObjectInBackgroundWithId:@"a397e97451" block:^(BmobObject *object,NSError *error){
        if (error){
            NSLog(@"%@",error);
        }else{
            if (object) {
...
```
    ...
    [bquery getObjectInBackgroundWithId:@"a397e97451" block:^(BmobObject *object,NSError *error){
        if (error){
            ...
            BmobObject *post = object;
            //获得BmobUser对象
            BmobUser *user = [BmobUser objectWithoutDatatWithClassName:@"_User" objectId:@"249560e723"];
...
```
    ...
    [relation addObject:[BmobObject objectWithoutDatatWithClassName:@"_User" objectId:@"6210011e51"]];
    [relation addObject:[BmobObject objectWithoutDatatWithClassName:@"_User" objectId:@"249560e723"]];
...
![](image/relationship_7.jpg)

从Post表中的title4记录点击关联关系框进去后查看的结果：

![](image/relationship_8.jpg)
...
```
    ...
    //新建relation对象
    BmobRelation *relation = [[BmobRelation alloc] init];
    [relation removeObject:[BmobObject objectWithoutDatatWithClassName:@"_User" objectId:@"6210011e51"]];
...
![](image/relationship_9.jpg)

#### 修改关联关系

如果需要给objectId为a397e97451的帖子添加多一个喜欢该帖子的读者可以使用以下代码

```
    BmobObject *author = [BmobObject objectWithoutDatatWithClassName:@"Post" objectId:@"a397e97451"];
    
    //新建relation对象
    BmobRelation *relation = [[BmobRelation alloc] init];
    [relation addObject:[BmobObject objectWithoutDatatWithClassName:@"_User" objectId:@"5f5cc4445a"]];
    //添加关联关系到postlist列中
...
![](image/relationship_10.jpg)

#### 查询关联关系

如果我们需要查询喜欢objectId为E3Nc222D的帖子的所有读者，可以采用下列代码：
...
```
结果如下，目前user3喜欢的帖子只有标题为title4的帖子

![](image/relationship_12.jpg)
...
```
[BmobUser loginInBackgroundWithAuthorDictionary:responseDictionary
                                       platform:BBmobSNSPlatformWeiXin
                                          block:^(BmobUser *user, NSError *error) {
                                           	NSLog(@"error%@",[error description]);
                                            }];
```
...
Bmob的文件管理经过迭代后目前有新旧两个版本，其中新版本可支持断点续传，适用于大文件的上传，而类似于头像文件等小图片文件上传，我们推荐使用旧文件上传，操作起来更加方便。

### 旧文件管理
#### 创建文件对象

BmobFile可以让你的应用程序将文件存储到服务器中，比如常见的文件类型图像文件，影像文件、音乐文件和任何其他二进制数据都可以使用。（单个文件大小不可超过10M），当文件上传成功后，可以通过url属性来获取文件的地址。

#### 上传文件

##### 上传文件方法

如下图的例子，是将3.txt的文本文件保存到服务器端：

```
NSArray *paths=NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
NSString *documentDirecotry=[paths objectAtIndex:0];
BmobFile *bmobFile = [[BmobFile alloc] initWithClassName:@"GameScore"
withFilePath:[documentDirecotry stringByAppendingPathComponent:@"3.txt"]];
//如果文件保存成功，则把文件添加到filetype列
if ([bmobFile save]) {
	[gameScore setObject:bmobFile forKey:@"filetype"];
} 
[gameScore saveInBackgroundWithResultBlock:^(BOOL isSuccessful,NSError *error){
	NSLog(@"%i",isSuccessful);
}];
```
...
#### 缩略图处理
Bmob也提供了几种简单的图片处理功能(对应的类为BmobImage)，下面就来介绍各自方法的功能**（一般来说，我们建议要处理的图片是已经上传到Bmob服务器上的，可以最大程度避免获取图片失败的情况发生）**：
...
* quality   图片的质量1~100
* imageUrl  原图片的地址
* type      返回类型：type为kBmobImageOutputBmobFile，返回的是BmobFile对象，为kBmobImageOutputStringStream，返回的是base64编码的字符串流
* block     生成返回结果和信息
 ...
 *  sourceImageUrl    原图的路径
 *  watermarkImageUrl 水印图的路径
 *  dissolve          透明度,范围在0-255
 *  direction         水印的位置
 *  x                 横轴边距，单位为像素，缺省为10
 *  y                 纵轴边距，单位为像素，缺省为10
 *  type              返回类型：type为kBmobImageOutputBmobFile，返回的是BmobFile对象，为kBmobImageOutputStringStream，返回的是base64编码的字符串流
 *  block             生成的BmobFile文件，及错误信息



### 新文件管理

使用新文件管理功能前先在需要使用的文件中导入以下头文件
...
1. 新版文件上传为了预防客户端重复上传文件，有做了简单地检查措施。目前的做法是根据文件的路径来做判断的。当文件上传后，会在本地的数据库里加入这个文件的上传记录。开发者上传文件时，会先读取数据库的内容，如果存在数据并且上传成功，则会直接返回之前上传成功后的文件名和文件的链接地址。
2. 文件上传中得到的url只是最终的访问url的一部分，完整的url构造请查看文件下载章节。
3. 为了兼容旧文件系统，从1.6.0版本开始，文件上传的回调添加了BmobFile对象返回值，用户可以直接保存该对象，也可以使用BmobFile值中的url属性来直接访问文件。


##### 单个文件上传

&#160; &#160; &#160; &#160;单个文件上传有两种方式，一种是通过文件路径进行上传，一种是通过NSData进行上传。样例代码如下：
...
##### 批量文件上传
&#160; &#160; &#160; &#160;批量文件上传也分别有通过路径上传以及通过NSData上传两种形式。样例代码如下：
...
#### 文件下载
&#160; &#160; &#160; &#160;文件下载有两种形式，一种是利用文件名下载，一种是利用url下载。
...
```
注意：
1. 传入的文件名为上传文件时返回的文件名而不是后台显示的文件名；
2. 下载的文件存放路径为：Library/Caches/DownloadFile/

##### 利用url下载
&#160; &#160; &#160; &#160;利用url下载的思路就是先获得可以访问的url，然后再利用iOS SDK或者开源库提供的方法传入url进行下载即可。而开启secretKey和关闭secretKey的情况下使用的url是不同的。下图红框处为开启SecretKey的地方，`注意开启勾选或取消开启后点击保存`。
![](image/file_3.jpg)




* 未开启SecretKey的url结构如下：
...
其中
t:客户端类型（1~19代表SDK客户端，1：IOS，2：Android，3：wp，…，20：官网后台；21：其他网站）
a:应用的AccessKey，位置如下图

![](image/file_4.jpg)

* 开启SecretKey的url可直接使用以下方法获得
...
按照以上方法获得url后即可利用相关的网络请求类进行下载。


#### 获取文件直接访问地址

未兼容旧版本前的上传的文件是没有直接访问url的，为了解决这个问题可以使用以下接口重新获得
...
#### 缩略图
缩略图可以对图片进行处理，可以在本地处理，也可以在服务器上进行处理。
...
```
    ...
    [BmobProFile thumbnailImageWithFilename:@"4E80C2958534450DB1D5DF5A97777A0A.jpg" ruleID:2 resultBlock:^(BOOL isSuccessful, NSError *error, NSString *filename, NSString *url,BmobFile *bmobFile) {
        //缩略图的文件名
        NSLog(@"filename %@",filename);
        //缩略图的url地址
        NSLog(@"fileUrl  %@",url);
...
```
其中，ID代表在服务器上已经设置好的缩略图配置，如下图所示

![](image/file_5.jpg)

注意：该block是为了兼容文件上传的接口，并没有返回BmobFile值，只有filename和url有效。

##### 本地处理
本地处理提供以下两种方法，一种为使用服务器上设置好的配置，另外一种是直接在代码上编写配置，样例如下。
...
```
    ...
    [BmobProFile localThumbnailImageWithFilepath:path ruleID:2 resultBlock:^(BOOL isSuccessful, NSError *error, NSString *filepath){
        //打印缩略图文件路径
        NSLog(@"filepath  %@",filepath);
    }];
```
...
```
    ...
    [BmobProFile localThumbnailImageWithFilepath:path mode:ThumbnailImageScaleModeWidth width:200 height:0 resultBlock:^(BOOL isSuccessful, NSError *error, NSString *filepath) {
        //打印缩略图文件路径
        NSLog(@"filepath %@",filepath);
    }];
```
...
这里有个[实例](http://static.bmob.cn/new/developmentdoc/appdemo/BmobDataDemo_iOS.zip)可以参考下。
...
```
要限制查询指定距离范围的数据可以使用whereWithinKilometers、whereWithinMiles或whereWithinRadians方法。 要查询一个矩形范围内的信息可以使用whereWithinGeoBox来实现：
``` 
...
关于云端代码的编写，详细参考  [云端代码开发文档](http://docs.bmob.cn/cloudcode/developdoc/index.html?menukey=develop_doc&key=develop_cloudcode)

## 短信服务
在一些应用场景下，你可能希望用户验证手机号码后才能进行一些操作，例如充值等。这些操作跟用户系统没有关系，可以通过我们提供的的短信验证API来实现。

每个 Bmob 帐户有 100 个免费额度的短信数量，超过需要购买短信条数才能继续使用。
