...
使用bmob有什么好处，下面一一为你道来：
 ...
 - 提供灵活的“云端代码”，其融合了bmob的数据服务和大量的第三方服务，让你轻松应对复杂的业务逻辑。同时云端代码能被第三方平台调用，再也不需要php，java等复杂的web服务。
 
本教程将引导你完成如下任务：

 - 创建bmob应用
 - 使用bmob云端代码实现微信公众平台的开发
...
在这个教程中，用到了bmob的两个功能：
 ...
 - 云端代码：微信后台回调云端代码后，完成公众号所需的业务逻辑：信息存储，把信息加工后返回给订阅者的微信。
...
这个工具有下面的功能：
 ...
 - 可选择以http "post"或者"get"的方法运行云端代码。
 - 选择request的参数和对应值：可添加传入云端代码的request的参数和对应值，参数的数目可以通过“再添加一个”按钮调整。
...
### 获取Secret Key
用户需要以http的方式运行云端代码，需要先确定应用的Secret Key。 调用云端代码时，通过Secret Key标识一个应用，获取Secret Key的路径：
...
点击保存后在云端代码调试区按“发送请求”，返回“success”的值，如下图16所示：

![](image/16.png)

在管理后台->数据浏览->应用表“message”下查看通过云端代码新增的数据，如下图17所示：
...
点击保存后在云端代码调试区按“发送请求”，返回表“message”的数据，如下图18所示：

![](image/18.png)

需要了解更多云端代码的操作，可阅读[http://docs.bmob.cn/cloudcode/developdoc/index.html?menukey=develop_doc&key=develop_cloudcode](http://docs.bmob.cn/cloudcode/developdoc/index.html?menukey=develop_doc&key=develop_cloudcode "云端代码开发文档")

在编写云端代码时有个注意事项：云端代码是使用异步编程。也就是说，当遇到文件读写请求，网络请求等IO操作时，代码不等待IO操作返回结果就执行后面的语句,当接收到IO操作的返回结果后才调用回调函数。
...
在服务员接待客人的行为中，通过比较普通的做法和异步的做法，能发现采用异步的方法后服务员的效率大大提高，云端代码使用异步也是基于同样的理由，当云端代码在等待IO操作（文件读写请求，网络请求）的结果时是一直空闲，如果不等待IO的结果继续执行下面的语句，能大大提高系统的效率。

在云端代码异步编程中“function(xxx){xxxx}”部分称为回调函数，云端代码会把IO操作的返回结果封装后传入到function函数执行里面的逻辑。
...
在本节中，通过bmob云端代码开发微信公众平台，实现一个反馈意见收集的功能：
...
function onRequest(request, response, modules) {
    ...
    var httptype = modules.oHttptype;　//获取调用云端代码的是post或者get方式
...
此处的URL（http://cloud.bmob.cn/a12af19a1b8bf434/feedback）为上节中生成的云端代码“feedback”的调用，按照云端代码的调用规格，a12af19a1b8bf434为该应用的Secret Key，标明调用的是哪个应用，feedback为云端代码的名称。Token定义为weixin。EncodingAESKey则不用填，点击“随机生成”让自动生成一个，消息加解密方式选择“明文模式”，然后点击“提交”按钮，如图22所示：
...
- 变为开发模式时，微信公众号后台往配置的url发送校验请求，这个过程云端代码校验信息的原理。
- 云端代码收发微信公众号后台传递过来的消息的原理。
...
在这个校验流程的云端代码中，使用oCrypto这个云端代码的加密对象模块，提供md5和sha1两种加密算法。通过这个模块，按照微信校验的流程完成校验。oCrypto更多的功能详细参考：https://www.npmjs.org/package/crypto

另外，云端代码使用了oHttptype模块获取当前的http调用方式。因为微信公众平台调用云端代码有两种方式：

- get方式，用于检验。
- post方式，用于转发订阅者往公众平台发送的消息。

通过oHttptype模块得知是用采用get方式调用云端代码，运行校验的代码并返回echostr参数。
...
从上图可以看出，用户在发送一个文本后，微信公众号后台将组装一个xml消息发送给云端代码服务器。当云端代码接收到http头部Content-Type为text/xml的请求后，云端代码自动把xml消息转换为一个对象放在request.body.xml中，通过获取request.body.xml对应的属性就能获取xml节点的值。

云端代码解析xml对象，根据节点信息，把发送者(request.body.xml.FromUserName)和消息内容（request.body.xml.Content）存储在表“message”后，然后通过一定的规则组装成一个xml文本回复给微信公众号后台，微信公众号后台再回复给用户。在这个收发过程中，发送方和接收方进行了调换(ToUserName和FromUserName值互换)，收发都是以xml格式在后台进行传输的。所以掌握各种消息类型的接收回复是进行微信公众平台开发的基础！
