...
需要了解更多云函数的操作，可阅读[https://docs.bmob.cn/cloudcode/WEB/a_faststart/doc/index.html](https://docs.bmob.cn/cloudcode/WEB/a_faststart/doc/index.html "云函数开发文档")
...
另外，云函数获取当前的http调用方式。因为微信公众平台调用云函数有两种方式：

- post方式，用于转发订阅者往公众平台发送的消息。

得知是用采用get方式调用云函数，运行校验的代码并返回echostr参数。
